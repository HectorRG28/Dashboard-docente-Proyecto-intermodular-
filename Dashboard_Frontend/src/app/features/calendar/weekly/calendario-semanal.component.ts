...
export class CalendarioSemanalComponent implements OnInit {
  ...
  busqueda = { fecha: '', inicio: '09:00', fin: '10:00' };
    ...
    tareas.forEach(t => {
      const f = new Date(t.fecha_inicio);
        if (f >= inicio && f <= fin) {
          ...
          // DETECTAR MENCIÓN
          if (t.ids_menciones && this.currentUserId) {
               const ids = t.ids_menciones.split(',').map((id:string) => parseInt(id.trim()));
  ...
  guardarTarea() {
    ...
    const p = {
      ...
      menciones: this.formularioTarea.menciones, // NUEVO
  ...
  borrarTareaDirecta(e: Event, id: number) {
    e.stopPropagation();
    this.modalService.openConfirm('¿Estás seguro de que deseas eliminar esta tarea?', () => {
      this.calendarService.deleteActividad(id).subscribe(() => this.cargarDatosSemana());
    });
  }
  ...
  borrarTareaDesdeModal() { 
    this.modalService.openConfirm('¿Eliminar esta tarea?', () => {
  ...
  private resetearFormulario() {
    this.formularioTarea = { id: null, titulo: '', descripcion: '', aula: '', menciones: [], fecha: this.fechaSeleccionada.toISOString().split('T')[0], horaInicio: '09:00', horaFin: '', id_modulo_seleccionado: null, id_tipo: null };
  }
  ...
  abrirBusqueda() {
    ...
    this.busqueda.fecha = this.fechaSeleccionada.toISOString().split('T')[0];
  ...
  buscarAulas() {
    this.calendarService.getAvailableAulas(this.busqueda.fecha, this.busqueda.inicio, this.busqueda.fin)
      .subscribe({
        ...
        error: (e) => this.modalService.open('Error buscando aulas', 'error')
      });
  }
  ...
  // ABRIR MODAL MENCIONES
  abrirModalMenciones() {
      const todas = [...this.lunes, ...this.martes, ...this.miercoles, ...this.jueves, ...this.viernes];
      this.listaMenciones = todas.filter(t => t.esMencion);
      this.modalMencionesAbierto = true;
  }
  cerrarModalMenciones() { this.modalMencionesAbierto = false; }
}