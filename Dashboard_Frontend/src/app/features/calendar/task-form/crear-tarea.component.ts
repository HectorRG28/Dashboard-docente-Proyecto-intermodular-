...
export class CrearTareaComponent implements OnInit {
  ...
  constructor(private calendarService: CalendarService, private router: Router, private modalService: ModalService) { }
  ...
  guardarTarea() {
    ...
    this.calendarService.createActividad(payload).subscribe({
      next: (res: any) => {
        this.modalService.open('¡Tarea guardada con éxito!', 'success');
        this.router.navigate(['/dashboard']); 
      },
      error: (err: any) => {
        console.error('Error al guardar:', err);
        this.modalService.open('Hubo un error al guardar.', 'error');
      }
    });
  }
